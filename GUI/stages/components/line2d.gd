...
var active: bool :
	set(value):
		...
		animate_line(get_parent().get_parent().stage_color) # seems duplicate from *_detail.draw_lines but both are needed
...
func add_points():
	...
	if (target.global_position.x - min_finish_length) < (origin.global_position.x + min_finish_length): # origin is further right than target
		var point: Vector2 = origin.global_position + Vector2(max(min_initial_length, target.global_position.x - origin.global_position.x), 0)
		add_point(point)
		if origin.global_position.y < target.global_position.y:
